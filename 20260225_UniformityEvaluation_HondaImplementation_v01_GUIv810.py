...
class ContourCut:
    ...
    def remove_small_local_residue(self):
        for ii in range(self.width):
            non_nan_count = np.count_nonzero(np.isnan(self.df_np[:,ii]))
            if non_nan_count > self.height-self.dy_threshold:
    ...
    def cut_along_Y_direction(self):
        ...
        self.remove_small_local_residue()
    ...
    def feature_pruning(self):
        ...
        for j in range(self.df_np.shape[1]):
            self.df_np[:, j] = remove_short_segments(self.df_np[:, j], self.dy_threshold)
