...
class ContourCut:
    ...
    def cut_along_X_direction(self):
        ...
        ymax_idx = np.argmax(self.lower_contour_y)
        ## find left / right contour X/Y-positions
        left_contour_x = self.lower_contour_x[ymax_idx:-1]
        left_contour_y = self.lower_contour_y[ymax_idx:-1]
        right_contour_x = self.lower_contour_x[0:ymax_idx+1]
        right_contour_y = self.lower_contour_y[0:ymax_idx+1]
        ## extract only the outmost position of X for left side
        idx = np.lexsort((left_contour_x, left_contour_y))
        y_sorted = left_contour_y[idx]
        first = np.r_[True, y_sorted[1:] != y_sorted[:-1]]
        keep_idx = idx[first]
        left_contour_x = left_contour_x[keep_idx]
        left_contour_y = left_contour_y[keep_idx]
        ## extract only the outmost position of X for right side
        idx = np.lexsort((-right_contour_x, right_contour_y))
        y_sorted = right_contour_y[idx]
        first = np.r_[True, y_sorted[1:] != y_sorted[:-1]]
        keep_idx = idx[first]
        right_contour_x = right_contour_x[keep_idx]
        right_contour_y = right_contour_y[keep_idx]
    ...
    def clean_edge(self):
        '''Clean up protruding edge.'''

        def remove_short_segments(arr, min_length):
            arr = arr.copy()
            isnan = np.isnan(arr)
            valid = ~isnan

            changes = np.diff(np.concatenate(([0], valid.view(np.int8), [0])))
        ...
        for j in range(self.df_np.shape[1]):
            self.df_np[:, j] = remove_short_segments(self.df_np[:, j], 5)
